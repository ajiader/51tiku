...
class diypage extends admin 
{
	...
	{
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m='.ROUTE_M.'&c='.ROUTE_C.'&a=add\', title:\'添加页面\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', '添加页面');
...
/**
	...
	private function check($data = array(), $a = 'add') {
		if($data['title']=='' or $data['dirname']=='') showmessage('标题或者目录不能为空！');
